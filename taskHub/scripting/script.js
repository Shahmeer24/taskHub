const TaskManager = {
  ...
  initializeTheme() {
    if (this.elements.themeToggle && this.elements.themeIcon) {
      ...
      this.elements.themeIcon.className = savedTheme === "dark" ? "fas fa-sun" : "fas fa-moon";
  ...
  toggleNoTasksMessage() {
    ...
    if (!noTasksMessage) {
      ...
      noTasksMessage.textContent = "yayy, no tasks left! Hit + to create a task.";
      document.body.appendChild(noTasksMessage);
    }

    noTasksMessage.style.display = Object.keys(this.tasksData).length === 0 ? "block" : "none";
  }
};
